...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { userId, userName, newExpiresAt, months } = await req.json();
    ...
    const formattedDate = new Date(newExpiresAt).toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' });
    const emailHtml = `<!DOCTYPE html><html><body style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto;"><div style="background: linear-gradient(135deg, #1a1a2e, #16213e); padding: 30px; text-align: center;"><h1 style="color: #fff;">EDAN</h1></div><div style="padding: 30px; background: #fff; border: 1px solid #e0e0e0;"><h2 style="color: #2ecc71;">¡Suscripción Renovada! 🔄</h2><p>Hola <strong>${userName}</strong>,</p><p>Tu suscripción ha sido renovada por <strong>${months} ${months === 1 ? 'mes' : 'meses'}</strong>.</p><p style="font-size: 20px; color: #2ecc71;"><strong>Nueva fecha: ${formattedDate}</strong></p><p><strong>El equipo de EDAN</strong></p></div></body></html>`;
