...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { userId, userName, previousStatus }: ReactivationEmailRequest = await req.json();
    ...
    const previousStatusText = previousStatus === "suspended" ? "suspendida" : 
                               previousStatus === "expired" ? "expirada" : 
                               previousStatus === "cancelled" ? "cancelada" : "inactiva";
    ...
    const emailHtml = `
      ...
      <body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #ffffff; padding: 30px; border: 1px solid #e0e0e0; border-top: none;">
          <h2 style="color: #2ecc71; margin-top: 0;">¡Tu cuenta ha sido reactivada! 🎉</h2>
          
          <p>Hola <strong>${userName}</strong>,</p>
          
          <p>¡Excelentes noticias! Tu cuenta que estaba ${previousStatusText} ha sido <strong>reactivada</strong>.</p>
          
          <div style="background: #d4edda; border: 1px solid #28a745; border-radius: 5px; padding: 15px; margin: 20px 0;">
            <p style="margin: 0; color: #155724;">
              <strong>✅ Tu acceso ha sido restaurado</strong><br>
              Ya puedes volver a disfrutar de todos los beneficios de tu membresía EDAN.
            </p>
          </div>
          
          <div style="background: #f8f9fa; border-radius: 8px; padding: 20px; margin: 20px 0;">
            <h3 style="margin-top: 0; color: #1a1a2e;">Ahora tienes acceso a:</h3>
            <ul style="margin: 0; padding-left: 20px;">
              <li>Todos los cursos de tu plan</li>
    ...
    const emailResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [userEmail],
        subject: "¡Tu cuenta EDAN ha sido reactivada! 🎉",
        html: emailHtml,
      }),
    });
    ...
    console.error("Error in send-reactivation-email function:", error);
    return new Response(
