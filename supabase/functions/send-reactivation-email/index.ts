...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { userId, userName, previousStatus } = await req.json();
    ...
    const statusText = previousStatus === "suspended" ? "suspendida" : previousStatus === "expired" ? "expirada" : "inactiva";
    const emailHtml = `<!DOCTYPE html><html><body style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto;"><div style="background: linear-gradient(135deg, #1a1a2e, #16213e); padding: 30px; text-align: center;"><h1 style="color: #fff;">EDAN</h1></div><div style="padding: 30px; background: #fff; border: 1px solid #e0e0e0;"><h2 style="color: #2ecc71;">¡Tu cuenta ha sido reactivada! 🎉</h2><p>Hola <strong>${userName}</strong>,</p><p>Tu cuenta que estaba ${statusText} ha sido <strong>reactivada</strong>. Ya puedes disfrutar de todos los beneficios.</p><p><strong>El equipo de EDAN</strong></p></div></body></html>`;
