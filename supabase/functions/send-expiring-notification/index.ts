...
const EMAIL_TYPE = "expiring_notification";
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { userId, userName, daysRemaining, expiresAt } = await req.json();
    ...
    const emailHtml = `<!DOCTYPE html><html><body style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto;"><div style="background: linear-gradient(135deg, #1a1a2e, #16213e); padding: 30px; text-align: center;"><h1 style="color: #fff;">EDAN</h1></div><div style="padding: 30px; background: #fff; border: 1px solid #e0e0e0;"><div style="background: ${urgencyColor}; color: white; padding: 10px; text-align: center; margin-bottom: 20px;"><strong>${daysRemaining <= 1 ? "¡Último día!" : "Recordatorio"}</strong></div><p>Hola <strong>${userName}</strong>,</p><p>Tu suscripción vence en <strong>${daysRemaining} día${daysRemaining > 1 ? 's' : ''}</strong> (${formattedDate}).</p><p><strong>El equipo de EDAN</strong></p></div></body></html>`;
