...
// Si no hay service key en .env, intentar obtenerla de supabase/config.toml
if (!supabaseServiceKey) {
  try {
    const configPath = join(process.cwd(), 'supabase', 'config.toml');
    const configContent = readFileSync(configPath, 'utf8');
    const match = configContent.match(/service_key\s*=\s*"([^"]+)"/);
    if (match && match[1]) {
      process.env.SUPABASE_SERVICE_ROLE_KEY = match[1];
    }
  } catch (error) {
    console.log('No se pudo leer supabase/config.toml:', error.message);
  }
}
